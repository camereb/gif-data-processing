...
deaths_under_5 <- 
  ...
  filter(
    REF_DATE >= 2015,
    Sex == "Both sexes",
    `Hierarchy for Age at time of death` == "1.2"
    ) %>%
  mutate(
...
total_live_births <- 
  live_births %>%
    filter(
      REF_DATE >= 2015,
      `Hierarchy for GEO` == "1",
      `Hierarchy for Geography, place of occurrence` != "1"
      ) %>%
  mutate(
    GEO = str_remove(`Geography, place of occurrence`, ", place of occurrence")
  ) %>%
  select(Year = REF_DATE, Geography = GEO, total_live_births = VALUE)
...
u5_mort_rate <- 
  ...
  left_join(total_live_births) %>%
    # Calculate mort rate
  transmute(Year, Geography, Value = round((deaths_u5/total_live_births)*1000, 2)) %>%
  filter(Geography != "Unknown province or territory of residence") %>%
    # Create total line
  mutate(Geography = ifelse(Geography == "Canada", "", Geography))

write_csv(u5_mort_rate, here("gif-data-processing", "data", "indicator_3-2-1.csv"))
